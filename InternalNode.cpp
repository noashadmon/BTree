...
InternalNode* InternalNode::insert(int value) //ALSMOST DONE LOOK TO SEE WHY I BECOMES NEGATIVE COUNT >=2
{
  ...
  if (isSiblingAvailable(leftSibling)) 
  {
  ...
  {
    if (isSiblingAvailable(rightSibling)) 
    {
...
bool InternalNode::isSiblingAvailable(BTreeNode *sibling)
{
  if (sibling && sibling->getCount() < internalSize)
    return true;
  return false;
}
